...
import ECB_Projects.boodschappenLijstje.Model.Winkels.dao.WinkelsDAO;
...
public class WinkelResource {
	...
	public Winkel getWinkel(@Auth Persoon user, @QueryParam("name") String Naam) {
		logger.info(String.format(
				"%s has requested personal  information on shop %s",
				user.getName(), Naam));
	...
	public int getId(@Auth Persoon user) {
		logger.info(String.format("%s has requested new ID", user.getName()));
	...
	public ArrayList<Winkel> getAllWinkel(@Auth Persoon user) {
		logger.info(String.format(
				"%s has requested personal  information onall shops",
				user.getName()));
	...
	public void create(@Auth Persoon user, Winkel newstore) {
		logger.info(String.format(
				"%s has requested to add shop %s",
				user.getName(), newstore.getNaam()));
		/*
		 * if (user instanceof Beheerder) { dao.insert(newstore); } else {
		 * 
	...
	@Timed
	@Path("/put")
	public void update(@Auth Persoon user, @QueryParam("name") String Naam,
			Winkel updatedwinkel) {
		logger.info(String.format(
				"%s has requested to update shop %s",
				user.getName(), Naam));
		/*if (user instanceof Beheerder) {
